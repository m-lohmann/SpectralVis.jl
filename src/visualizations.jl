...
"""
function gamut_vis(type, title, lum, whitepoint, colmatch,name::AbstractString,altitude::Real,slice=collect(5:5:95))
    println("Preparing gamut... this may take about 30s.")
    a,b,c=sl(type,lum,whitepoint,colmatch,390.0,1.0,830.0)
    println("Creating animation frames... this may take a few seconds.")
    n=name
    if type == xyY
        xl = (0,1)
        yl = (0,1)
...
function gamut_vis(type,T1::Real,dT::Real,T2::Real,colmatch,name::AbstractString,altitude::Real,azimuth::Real,slice=collect(5:5:95))
    ...
    for T in T1:dT:T2
        ...
        println("Creating animation frames... this may take a few minutes.")
        n=name
        if type == xyY
            xl = (0,1)
            yl = (0,1)
        ...
        gamutslice(type, title, a, b, c,(xl,yl,zl,xg,yg,zg), azimuth, altitude, slice)
        png("$(name)_$(T)")
        println("Frame for CCT $T K finished.")
    end
end
...
function gamutslice(type, title, a, b, c, (xl,yl,zl,xg,yg,zg), azimuth, altitude, slice)
    ...
    @inbounds for s in slice
        ...
        scatter!(h,i,j,labels=false,color=:black,linewidth=0.5,markersize=0.35,xlims=xl,xguide=xg,ylims=yl,yguide=yg,zlims=zl,zguide=zg,camera=(azimuth,altitude),title="$(title)")
    end
end